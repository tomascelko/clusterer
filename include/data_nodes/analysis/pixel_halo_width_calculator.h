...
class pixel_halo_width_calculator: public i_simple_consumer<cluster_type<hit_type>>
{
    ...
    bool check_square(const hit_type & hit, int32_t radius)
    {
        ...
        {
            neighbors_count += (check_pixel(hit.x() + i, hit.y() - radius)
             + check_pixel(hit.x() - radius, hit.y() + i)
             + check_pixel(hit.x() + radius, hit.y() + i)
             + check_pixel(hit.x() + i, hit.y() + radius));           
        }
    ...
    void process_cluster(const cluster_type<hit_type>& cl)
    {
        ...
        {
            if(hit.x() == 0 || hit.y() == 0 
            || hit.x() == current_chip::chip_type::size_x() - 1 
            || hit.y() == current_chip::chip_type::size_y() - 1)
    ...
    public:
    pixel_halo_width_calculator(stream_type * stream) :
    out_stream_(std::move(std::unique_ptr<stream_type>(stream))),
