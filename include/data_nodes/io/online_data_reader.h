...
class katherine_parser
{
    ...
    burda_hit parse_burda_hit(int64_t data, uint32_t offset)
    {
        ...
        uint16_t linear_coord = data >> 28;
        data = data & 0xfffffffL;
        int64_t toa = (data >> 14) + 16384 * offset;
        data = data & 0x3fffL;
        int16_t tot = data >> 4;
        uint8_t fast_toa =  data & 0xfL;
        auto hit =  burda_hit{linear_coord, toa, fast_toa, tot};
    ...
    bool parse_6byte_data_frame(int64_t data_frame, std::vector<burda_hit> & hits)
    {
        ...
        { 
            ...
            case measurement_data_type::KATHERINE_2_ID:
                hits.push_back(parse_burda_hit(data_frame, time_offset_)); 
                break;
...
class udp_controller
{
    ...
    const uint64_t UDP_BUFFER_SIZE = 65536;
    ...
    char* data_response = new char[UDP_BUFFER_SIZE]{0x0};
    ...
    void async_send_command(char* command)
    {
        ...
        //std::cout << std::endl;
        local_control_socket.async_send_to(boost::asio::buffer(command, 8), remote_control_endpoint, std::bind(&udp_controller::on_control_send, this, command_index, std::placeholders::_1, std::placeholders::_2));
        ++command_index;  
    }
    ...
    {
        bool acq_end = false;
        //std::cout << parser.total_hits_received<< " hits_received" << std::endl;
        ...
        if(!acq_end)
        {
           //std::cout << "Bystes transferred " << bytes_transferred <<std::endl;
           local_data_socket.async_receive_from(boost::asio::buffer(data_response, UDP_BUFFER_SIZE), sender_endpoint1, std::bind(&udp_controller::on_data_receive, this, std::placeholders::_1, std::placeholders::_2, sender_endpoint1));        
        }
        else
        {
    ...
    {
        setup_control();
        local_data_socket.set_option(boost::asio::socket_base::receive_buffer_size(UDP_BUFFER_SIZE));
    ...
    void start_acq()
    {
        ...
        boost::asio::ip::udp::endpoint sender_endpoint1;
        local_data_socket.async_receive_from(boost::asio::buffer(data_response, UDP_BUFFER_SIZE), sender_endpoint, std::bind(&udp_controller::on_data_receive, this, std::placeholders::_1, std::placeholders::_2, sender_endpoint));        
...
public:
    ...
    void end_acq()
    {
        std::cout << "TOTAL HITS PROCESSED: " << this->total_hits_read_ << std::endl;
        std::cout << "UDP TRANSFER SUCCESS RATE: " << (this->total_hits_read_ / 
    ...
    {      
        ...
        katherine_controller.start_acq();
        //while (!done)
        {
