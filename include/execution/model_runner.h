...
private:
  ...
  static std::string create_split_node(const node_args &args,
  ...
  static std::string create_split_arch(const node_args &args,
                                       const std::string &first_node,
                                       const std::string &cluster_node,
                                       uint32_t core_count) {
    std::string arch = "";
    std::vector<std::string> nodes_to_connect = {first_node, cluster_node};
    nodes_to_connect.insert(nodes_to_connect.begin() + 1,
                            preprocessing_nodes.begin(),
                            preprocessing_nodes.end());
    nodes_to_connect.insert(nodes_to_connect.end(),
                            postprocessing_nodes.begin(),
                            postprocessing_nodes.end());
    ...
    bool splitted = false;
    for (uint32_t node_index = 0; node_index < nodes_to_connect.size() - 1;
         ++node_index) {
  ...
  add_merge_arch(model_name model_type, std::string &arch,
    ...
    auto connecting_node = postprocessing_nodes.size() > 0
                               ? postprocessing_nodes.back()
                               : clustering_node;
    ...
    if (model_type == model_name::PAR_SIMPLE_MERGER) {
      ...
      for (uint16_t i = 1; i <= core_count; i++) {
        ...
        std::string merg_high =
            "m" + std::to_string(((i) % (core_count)) +
                                 1); // modulo in 1 based indexing
        arch += "," + sc + merg_low;
        arch += "," + sc + merg_high;
        if (output_node != "") {
      ...
      throw std::invalid_argument(
          "Specified model does not support explicit paralelism");
  }
  // facilitates the construction of the whole architecture recipe for
  // data-based split architectures
  static architecture_type
  build_parallel_arch(model_name model_type, const std::string &first_node,
                      uint16_t core_count, const std::string &output_node,
    ...
    add_merge_arch(model_type, arch, descriptors, clusterer_node, output_node,
                   core_count);
    return architecture_type(arch, descriptors);
  }

public:
  // the main method of the runner, based on the model type, run the
  // approperiate model building method
  template <typename executor_type, typename... clustering_args>
  static std::vector<std::string>
  run_model(model_name model_type, executor_type *executor, uint16_t core_count,
            node_args &args, bool debug_mode = false,
            clustering_type clustering_alg_type = clustering_type::STANDARD) {
    // std::cout << "Building the architecture..." << std::endl;
    std::string validation_arch = "rc1cv1,rc2cv1";

    auto multi_merge_descr_11 =
        new temporal_merge_descriptor<cluster<mm_hit>>(2, 1);
    auto multi_merge_descr_12 =
        new temporal_merge_descriptor<cluster<mm_hit>>(2, 1);
    auto multi_split_descr_11 =
        new temporal_merge_descriptor<cluster<mm_hit>>(2, 1);
    auto multi_split_descr_12 =
        new temporal_merge_descriptor<cluster<mm_hit>>(2, 1);

    auto multi_merge_descr_2 =
        new temporal_merge_descriptor<cluster<mm_hit>>(2, 2, false);
    auto multi_split_descr_2 =
        new temporal_merge_descriptor<cluster<mm_hit>>(2, 2, false);
    auto hit_sorter_split_descr =
        new temporal_hit_split_descriptor<mm_hit>(core_count);
    auto trivial_hit_merge_descr = new trivial_merge_descriptor<mm_hit>();
    ...
    case model_name::SIMPLE_CLUSTERER:
      return executor->run(architecture_type(arch + ",s1" + clustering_node +
                                             "1," + clustering_node + "1" +
                                             default_output_node),
                           args, debug_mode);
      break;
    ...
    case model_name::TRIG_BB_CLUSTERER:
      return executor->run(
          architecture_type(arch + ",s1trbbc1,trbbc1" + default_output_node),
          args, debug_mode);
      break;
    ...
    case model_name::VALIDATION:
      return executor->run(architecture_type(validation_arch), args,
                           debug_mode);
      break;
    case model_name::WINDOW_COMPUTER:
      if (print)
        return executor->run(architecture_type(arch + ",s1wfc1,wfc1wp1"), args,
                             debug_mode);
      else
        return executor->run(architecture_type(arch + ",s1wfc1,wfc1cow1"), args,
                             debug_mode);
      break;
    case model_name::TRIGGER_SIMPLE_CLUSTERER:
      sub_arch = arch.substr(0, arch.find(','));
      return executor->run(
          architecture_type(sub_arch + ",bm1tr1,tr1s1,s1" + clustering_node +
                            "1," + clustering_node + "1" + default_output_node),
          args, debug_mode);
      break;
    default:
