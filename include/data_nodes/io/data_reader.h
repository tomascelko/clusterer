...
class data_reader : public i_multi_producer<data_type, descriptor_type>,
                    public i_controlable_source {
protected:
  using buffer_type = data_buffer<data_type>;
  // sleep duration after the whole computational graph cannot load more data to
  // RAM after this time, the memory check is repeated
...
public:
  data_reader(descriptor_type *node_descriptor, const std::string &file_name,
              const node_args &args)
      : input_stream_(std::move(std::make_unique<istream_type>(file_name))),
        sleep_duration_(
            args.get_arg<double>(name(), "sleep_duration_full_memory")),
  ...
  data_reader(const std::string &file_name, const node_args &args)
      : input_stream_(std::move(std::make_unique<istream_type>(file_name))),
        sleep_duration_(
            args.get_arg<double>(name(), "sleep_duration_full_memory")),
        i_multi_producer<data_type, descriptor_type>() {
    check_input_stream(file_name);
  }

  virtual void pause_production() override { paused_ = true; }

  virtual void continue_production() override { paused_ = false; }
