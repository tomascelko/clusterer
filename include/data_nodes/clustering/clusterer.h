...
protected:
  const double CLUSTER_CLOSING_DT =
      300; // time after which the cluster is closed (> DIFF_DT, because of
           // delays in the datastream)
...
protected:
  ...
  void merge_clusters(unfinished_cluster<mm_hit> &bigger_cluster,
                      unfinished_cluster<mm_hit> &smaller_cluster)
  ...
  // unorderedness, can hovewer improve performance
  // try always merging to left (smaller toa)
  {
    ...
    bigger_cluster.cl.hits().reserve(bigger_cluster.cl.hits().size() +
                                     smaller_cluster.cl.hits().size());
    bigger_cluster.cl.hits().insert(
        bigger_cluster.cl.hits().end(),
        std::make_move_iterator(smaller_cluster.cl.hits().begin()),
        std::make_move_iterator(
            smaller_cluster.cl.hits()
                .end())); // TODO try hits in std list and then concat in O(1)

    // merge clusters
    bigger_cluster.pixel_iterators.reserve(
        bigger_cluster.pixel_iterators.size() +
        smaller_cluster.pixel_iterators.size());
    bigger_cluster.pixel_iterators.insert(
        bigger_cluster.pixel_iterators.end(),
        std::make_move_iterator(smaller_cluster.pixel_iterators.begin()),
        std::make_move_iterator(
            smaller_cluster.pixel_iterators
                .end())); // TODO try hits in std list and then concat in O(1)

    // update first and last toa
    bigger_cluster.cl.set_first_toa(std::min(bigger_cluster.cl.first_toa(),
                                             smaller_cluster.cl.first_toa()));
    bigger_cluster.cl.set_last_toa(
        std::max(bigger_cluster.cl.last_toa(), smaller_cluster.cl.last_toa()));

    unfinished_clusters_.erase(smaller_cluster.self);
    --unfinished_clusters_count_;
  }
  std::vector<cluster_it>
  find_neighboring_clusters(const coord &base_coord, double toa,
                            cluster_it &biggest_cluster) {
    ...
    for (auto neighbor_offset : EIGHT_NEIGHBORS) // check all neighbor indexes
    {
      ...
      // cannot two "already mergable" clusters on a single neighbor pixel
      {
        if (std::abs(toa - neighbor_cl_it->cl.last_toa()) < cluster_diff_dt &&
            !neighbor_cl_it->selected) // TODO order
...
public:
  ...
  std::string name() override { return "clusterer"; }
  void set_manual_writing() { manual_write_check_ = true; }
  void set_auto_writing() { manual_write_check_ = false; }
  ...
  std::vector<cluster<mm_hit>> retrieve_old_clusters(double hit_toa = 0) {
    ...
    while (unfinished_clusters_count_ > 0 &&
           ...
           i++) // update iterator
      {
        ...
        pixel_list_row.erase(
            current.pixel_iterators[i]); // FIXME pixel_list_rows should be
                                         // appended in other direction
      }
  ...
  pixel_list_clusterer(descriptor_type *node_descr, const node_args &args)
      : pixel_lists_(current_chip::chip_type::size_x() *
        ...
        unfinished_clusters_count_(0), processed_hit_count_(0),
