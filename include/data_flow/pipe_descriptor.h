...
public:
  inline uint32_t pipe_count() { return 0; };
...
public:
  inline uint32_t get_pipe_index(const data_type &data) { return 0; };
...
public:
  inline bool is_on_border(const data_type &data) { return false; };
  inline bool should_be_forwarded(const data_type &data) { return false; };
...
public:
  inline uint32_t pipe_count() { return pipe_count_; }
...
public:
  ...
  inline uint32_t get_pipe_index(const hit_type &hit) {
    return (std::llround(hit.time()) % full_rotation_interval_) /
           SWITCH_INTERVAL_LEN;
  }

  inline bool is_on_border(const cluster<hit_type> &cl) {
  ...
  inline bool should_be_forwarded(const cluster<hit_type> &cl) { return false; }
...
public:
  ...
  inline uint32_t get_pipe_index(const data_type &hit) { return 0; }
  inline uint32_t pipe_count() { return 1; }
...
public:
  temporal_hit_split_descriptor(uint32_t pipe_count)
      : pipe_count_(pipe_count),
        full_rotation_interval_(pipe_count * SWITCH_INTERVAL_LEN) {}
  inline uint32_t get_pipe_index(const data_type &hit) {
    return (std::llround(hit.time()) % full_rotation_interval_) /
           SWITCH_INTERVAL_LEN;
  }
  inline uint32_t pipe_count() { return pipe_count_; }
...
public:
  ...
  inline bool is_on_border(const data_type &cl) { return false; }
  inline bool should_be_forwarded(const data_type &cl) { return false; }
  inline uint32_t pipe_count() { return 1; }
...
class temporal_merge_descriptor
  ...
  uint32_t depth_;
  bool inner_node_;
  inline bool is_on_border_multiple(const cluster_type &cl, uint32_t multiple) {
...
public:
  temporal_merge_descriptor(uint32_t pipe_count, uint32_t depth,
      ...
      : merge_split_descriptor<cluster_type, cluster_type>(pipe_count),
        ...
        full_rotation_interval_(pipe_count * SWITCH_INTERVAL_LEN) {}
  inline uint32_t get_pipe_index(const cluster_type &cl) {
    if (inner_node_ && should_be_forwarded(cl))
      return 1;
    return 0;
  }
  inline bool is_on_border(const cluster_type &cl) {
    return is_on_border_multiple(cl, 1);
  }
  inline bool should_be_forwarded(const cluster_type &cl) {
...
public:
  inline uint32_t get_pipe_index(const cl_type &cl) {
    uint32_t remainder =
        std::abs(std::llround(cl.first_toa()) % SWITCH_INTERVAL_LEN);
  ...
  inline uint32_t pipe_count() { return 2; }
