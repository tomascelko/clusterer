...
template <typename data_type> class data_block {
...
public:
  bool is_fixed_size() const { return fixed_size_; }

  uint64_t byte_size() { return byte_size_; }
  uint64_t size() { return data_block_.size(); }
  static constexpr uint32_t max_block_byte_size() { return 2 << 15; }
  static constexpr double closing_time() { return 200.; }
  data_block(uint64_t avg_data_size = data_type::avg_size())
      : byte_capacity_(data_block<data_type>::max_block_byte_size()),
        to_delete_index_(0), fixed_size_(true) {
    data_block_.reserve(2 * data_block<data_type>::max_block_byte_size() /
                        avg_data_size);
  }
  bool is_temporally_wide() {
    ...
    if (data_block_.size() > 0)
      return false;
    bool is_wide = (data_block_[data_block_.size() - 1].time() -
                    data_block_[0].time()) > MAX_TEMPORAL_DIFF;
    /*if (is_wide)
    {
       std::cout << "";
    }*/
  ...
  bool can_peek() { return to_delete_index_ != data_block_.size(); }
  const data_type &peek() { return data_block_[to_delete_index_]; }
...
template <typename data_type,
          template <typename> typename queue_type = moodycamel::ConcurrentQueue>
...
private:
  ...
  // two variants of emplace method allow for simple switching between
  // implementations of the queue as of our testing no significant performance
  // difference was observed between these implementations
  void
  emplace_impl(data_block<data_type> &&block,
               moodycamel::ReaderWriterQueue<data_block<data_type>> &queue) {
    queue_.emplace(block);
  }
  void emplace_impl(data_block<data_type> &&block,
                    moodycamel::ConcurrentQueue<data_block<data_type>> &queue) {
    queue_.enqueue(block);
  }

public:
  static constexpr uint64_t MAX_Q_LEN = 2ull << 31; // in bytes
  default_pipe(const std::string &name, i_data_node *producer = nullptr,
               i_data_node *consumer = nullptr)
      : name_(name), producer_(producer), consumer_(consumer) {}
  uint64_t processed_counter = 1;
  i_data_node *producer() override { return producer_; }
  i_data_node *consumer() override { return consumer_; }
  virtual uint64_t bytes_used() override { return bytes_used_; }
  ...
  void blocking_enqueue(data_block<data_type> &&new_block) {
    ...
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(100)); //a thread
    has
    }*/
    emplace_impl(std::move(new_block),
                 queue_); // TODO or call enqueue when using multi queue
  }
  ...
  uint32_t approx_size() { return queue_.size_approx(); }
