...
template <typename data_type> class pipe_writer {
...
public:
  ...
  pipe_writer(default_pipe<data_type> *pipe)
      : pipe_(pipe), block_(pipe_->mean_data_size()) {}
  default_pipe<data_type> *pipe() { return pipe_; }
  ...
  virtual uint32_t open_pipe_count() { return 1u; }
};