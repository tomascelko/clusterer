...
class repeating_data_reader : public data_reader<data_type, istream_type> {
  ...
  // instead of sorting data types, we sort only references (for quick
  // searching) this way we preserve the unsortedness in the real data
  struct data_type_wrapper_reference {
    data_type data;
    uint64_t index;
    data_type_wrapper_reference(const data_type &data_field, uint64_t index)
        : data(data_field), index(index) {}
  ...
  using buffer_type =
      typename data_reader<data_type, istream_type>::buffer_type;
  ...
  void offset_matching_hit(const mm_hit &hit, double new_time) {
    auto reverse_converted_hit =
        calibrator_->reverse_incomplete_convert_hit(hit);
    auto it =
        binary_find(rep_buffer_sorted_,
                    data_type_wrapper_reference{reverse_converted_hit, 0});
    ...
    short fast_ticks =
        std::round((slow_ticks * calibrator_->slow_clock_dt - new_time) /
                   calibrator_->fast_clock_dt);
  ...
  void run_clustering() {
    ...
    std::transform(rep_buffer_.begin(), rep_buffer_.end(),
                   std::back_inserter(converted_hits),
                   [this](auto hit) { return calibrator_->convert_hit(hit); });
    std::sort(converted_hits.begin(), converted_hits.end(),
    ...
    for (auto hit : converted_hits) {
      ...
      clusters_.insert(clusters_.end(), new_clusters.begin(),
                       new_clusters.end());
    }
    auto new_clusters =
        clusterer_->retrieve_old_clusters(std::numeric_limits<double>::max());
    clusters_.insert(clusters_.end(), new_clusters.begin(), new_clusters.end());

    for (uint64_t i = 0; i < rep_buffer_.size(); ++i) {
      rep_buffer_sorted_.push_back(
          data_type_wrapper_reference{rep_buffer_[i], i});
    }
  ...
  void init_read_data() {
    ...
    *this->input_stream_ >> data;
    ...
    while (data.is_valid() &&
           (processed_count < buffer_size_ || buffer_size_ <= 0)) {
  ...
  data_type offset_hit(const data_type &hit, uint64_t offset) {
    return data_type{hit.linear_coord(),
                     static_cast<int64_t>(std::round(hit.tick_toa() + offset)),
                     hit.fast_toa(), hit.tot()};
  }
  // initialize clustering for frequency scaling
  void initialize_internal_clustering(const std::string &calib_folder,
                                      const node_args &args) {
    const double EPSILON = 0.000001;
    if (calib_folder != "") {
      calibration calib =
          calibration(calib_folder, current_chip::chip_type::size());
      calibrator_ = std::move(
          std::make_unique<burda_to_mm_hit_adapter<mm_hit>>(std::move(calib)));
      clusterer_ =
          std::move(std::make_unique<pixel_list_clusterer<cluster>>(args));
    }
    if (calib_folder == "" && std::abs(freq_multiplier_ - 1) > EPSILON)
      throw std::invalid_argument("No calibration folder provided - it is "
                                  "required for frequency scaling");
  }
...
public:
  ...
  virtual void start() override {
    ...
    uint64_t toa_offset = (rep_buffer_[rep_buffer_.size() - 1].tick_toa() -
                           rep_buffer_[0].tick_toa());
    // repetition of the buffer
    for (uint32_t rep_index = 0; rep_index < repetition_count_; ++rep_index) {
      // loop over the buffer
      for (uint32_t hit_index = 0; hit_index < rep_buffer_.size();
           ++hit_index) {
        this->writer_.write(
            offset_hit(rep_buffer_[hit_index], toa_offset * rep_index));
