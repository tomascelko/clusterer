...
#include <queue>
#pragma once
// auxiliary bounding box around the cluster, to quickly separate non-mergeable
// clusters
struct bbox {
  coord left_top;
  coord right_bot;
  ...
  bbox(coord &&left_top, coord &&right_bot)
      : left_top(left_top), right_bot(right_bot) {}
  ...
  bbox(const cluster<hit_type> &cluster)
    ...
    // therefore the mergability condition can be represented as a intersection
    // check
  ...
  bool overlaps_with(const bbox &other) const {
    if (right_bot.x() < other.left_top.x() ||
        other.right_bot.x() < left_top.x())
      return false;
    if (right_bot.y() < other.left_top.y() ||
        other.right_bot.y() < left_top.y())
      return false;
    return true;
  }
  ...
  bool lies_in_bb(coord coordinate) const {
    return coordinate.x() >= left_top.x() && coordinate.y() >= left_top.y() &&
           coordinate.x() <= right_bot.x() && coordinate.y() <= right_bot.y();
  }
  uint32_t size_x() const { return right_bot.x() - left_top.x(); }
  uint32_t size_y() const { return right_bot.y() - left_top.y(); }
  uint32_t area() const { return size_x() * size_y(); }
};
// an auxiliary wrapper around the cluster which contains additional cluster
// metadata
template <typename hit_type,
          typename cluster_reference_type = cluster<hit_type>>
struct bb_cluster {
  // cluster
  ...
  bb_cluster(cluster_reference_type &&cl, uint32_t bordering_offset = 0,
             bool should_compute = true)
      : // CHECK ME
        cl(cl), bordering_offset_(bordering_offset),
...
class cluster_merging : public i_data_consumer<cluster<hit_type>>,
  ...
  const double MAX_CLUSTER_TIMESPAN =
      2000.; // maximum total timespan of a cluster, must be set to speedup
             // merging
  const std::vector<coord> NINE_NEIGHBORS = {
      {-1, -1}, {-1, 0}, {-1, 1}, {0, -1}, {0, 0},
      {0, 1},   {1, -1}, {1, 0},  {1, 1},  {0, 0}};
  ...
  std::priority_queue<cluster<hit_type>, std::vector<cluster<hit_type>>,
                      typename cluster<hit_type>::first_toa_comparer>
      priority_queue_;
  ...
  bool are_spatially_neighboring(const bb_cluster<hit_type> &bb_cl_1,
    ...
    const bb_cluster<hit_type> &bigger_bb_cl =
        bb_cl_1.cl.hits().size() < bb_cl_2.cl.hits().size() ? bb_cl_2 : bb_cl_1;
    const bb_cluster<hit_type> &smaller_bb_cl =
        bb_cl_1.cl.hits().size() < bb_cl_2.cl.hits().size() ? bb_cl_1 : bb_cl_2;
      ...
      if (intersection_bb.lies_in_bb(hit.coordinates())) {
        for (auto &neighbor_offset : NINE_NEIGHBORS) {
          if (hit.coordinates().is_valid_neighbor(neighbor_offset) &&
              (neighboring_matrix_[hit.x() + neighbor_offset.x()]
                                  [hit.y() + neighbor_offset.y()] >= 0) &&
              std::abs(neighboring_matrix_[hit.x() + neighbor_offset.x()]
                                          [hit.y() + neighbor_offset.y()] -
                       hit.toa()) < merge_time) {
            is_neighbor = true;
            break;
          }
  ...
  bool can_merge(const bb_cluster<hit_type> &bb_cl_1,
                 const bb_cluster<hit_type> &bb_cl_2) {
    ...
    return are_spatially_neighboring(bb_cl_1,
                                     bb_cl_2); // full proper neighboring check
  }
  ...
  void process_border_cluster(cluster<hit_type> &old_cl) {
    ...
    for (int32_t i = unfinished_border_clusters_.size() - bordering_offset;
         i >= 0; i -= unfinished_border_clusters_[i].bordering_offset()) {
      if (unfinished_border_clusters_[i].valid &&
          unfinished_border_clusters_[i].cl.first_toa() <
              bb_cl.cl.first_toa() - (MAX_CLUSTER_TIMESPAN + merge_time))
        break;
      if (unfinished_border_clusters_[i].valid &&
          can_merge(unfinished_border_clusters_[i], bb_cl)) {
    ...
    if (merge_indices.size() == 0)
      ...
      uint32_t oldest_to_merge =
          merge_indices.back(); // retrieve oldest cluster for merging
      merge_indices.pop_back();
      unfinished_border_clusters_[oldest_to_merge].merge_with(
          std::move(bb_cl));               // merge current with oldest
      for (auto merge_ind : merge_indices) // possibly also merge other suitable
                                           // clustrers with the oldest
      {
        unfinished_border_clusters_[oldest_to_merge].merge_with(
            std::move(unfinished_border_clusters_[merge_ind]));
  ...
  void remove_invalid() {
    while ((!unfinished_border_clusters_.empty()) &&
           !unfinished_border_clusters_.front().valid)
  ...
  void write_old_clusters(double last_toa) {
    ...
    while ((!unfinished_border_clusters_.empty()) &&
           (!unfinished_border_clusters_.front().bordering() ||
            unfinished_border_clusters_.front().cl.last_toa() <
                last_toa - merge_time)) {
  ...
  void write_remaining_clusters() {
    while ((!unfinished_border_clusters_
                 .empty())) // process last clusters on border
    {
      remove_invalid();
      if (!unfinished_border_clusters_.empty()) {
        this->writer_.write(std::move(unfinished_border_clusters_.front().cl));
