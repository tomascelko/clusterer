    ...
    void clusterize_by_threshold(double threshold)
    {
        ...
        for (uint32_t i = 0; i < hit_buffer_.size(); i++)
        {
            ...
            if (hit_buffer_valid_[i])
            {
                ...
                while (bb_current_index >= 0 && open_clusters_its[bb_current_index]->cl.first_toa() - hit_buffer_[i].toa() < MIN_CLUSTERING_DT && hit_buffer_[i].toa() - open_clusters_its[bb_current_index]->cl.last_toa() < MIN_CLUSTERING_DT)
                {
...
public:
    ...
    halo_buffer_clusterer(const node_args &args) : clusterer_(std::make_unique<clusterer_type>(args)),
