...
struct node_descriptor : public abstract_node_descriptor
{
    ...
    node_descriptor(merge_descriptor<merge_type> *merge_descr, split_descriptor<split_type> *split_descr,
                    const std::string &descr_id) : merge_descr(merge_descr),
...
public:
    ...
    temporal_clustering_descriptor(uint32_t pipe_count) : merge_split_descriptor<cluster<hit_type>, hit_type>(pipe_count),
    ...
    bool is_on_border(const cluster<hit_type> &cl) override
    {
        ...
        return first_remainder < EPSILON_BORDER_TIME || first_remainder > SWITCH_INTERVAL_LEN - EPSILON_BORDER_TIME || last_remainder < EPSILON_BORDER_TIME || last_remainder > SWITCH_INTERVAL_LEN - EPSILON_BORDER_TIME || cl.last_toa() - cl.first_toa() > SWITCH_INTERVAL_LEN;
    }
...
public:
    temporal_hit_split_descriptor(uint32_t pipe_count) : pipe_count_(pipe_count),
                                                         full_rotation_interval_(pipe_count * SWITCH_INTERVAL_LEN)
    {
    }
    uint32_t get_pipe_index(const data_type &hit) override
...
public:
    temporal_merge_descriptor(uint32_t pipe_count, uint32_t depth, bool inner_node = true) : merge_split_descriptor<cluster_type, cluster_type>(pipe_count),
                                                                                             depth_(depth),
