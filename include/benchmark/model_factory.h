...
struct node
{
    std::string type;
    uint32_t id;
    node(const std::string &type, uint32_t id) : type(type),
                                                 id(id) {}
...
public:
    architecture_type(const std::string &arch_string, uint32_t tile_size = 1, double window_size = 50000000) : window_size_(window_size),
                                                                                                               tile_size_(tile_size)
    {
        init_arch(arch_string);
    }

    architecture_type(const std::string &arch_string, std::map<std::string, abstract_node_descriptor *> node_descriptors, uint32_t tile_size = 1, double window_size = 50000000) : node_descriptors_(node_descriptors),
