...
public:
    ...
    std::vector<std::string> trigger_files{
        "trigger/trained_beam_change.nnt",
        "trigger/trained_beam_change.svmt",
        "trigger/trained_beam_change.osvmt",
    };
    ...
    void prepend_path(const std::string &prefix_dir)
    {
        ...
        if (!prefix_dir[prefix_dir.size() - 1] == '/' && !prefix_dir[prefix_dir.size() - 1] == '\\')
            prefix = prefix_dir + '/';
...
class performance_test
{
    struct statistics
    {
        double mean;
        double std;
        statistics(double mean, double std) : mean(mean),
                                              std(std) {}
    };
...
public:
    ...
    void run_all_benchmarks(const std::string &binary_path, std::string &dataset_path, uint64_t core_count = 4)
    {
        ...
        char last_path_char = dataset_path[dataset_path.size() - 1];
        if (last_path_char != '/' && last_path_char != '\\')
            dataset_path += '/';
        dataset_.prepend_path(dataset_path);
